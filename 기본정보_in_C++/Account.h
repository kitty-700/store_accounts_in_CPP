...
class Account {
	...
	char ID[buffer::id_length];
	char PW[buffer::password_length];
	char update_time[buffer::update_time_length];	//마지막 업데이트 일자
	char memo[buffer::memo_length];
private:
	bool is_proper_string(std::string what_attribute, std::string str) const;
	void replace_account_update_time(char now_time[buffer::update_time_length]);
public:
