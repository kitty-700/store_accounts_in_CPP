...
void Site::del_account(std::string ID)
{
	for (std::list<Account*>::iterator each = this->accounts.begin(); each != this->accounts.end(); each++)
	{
		if ((*each)->get_attribute("ID") == ID)
		{
			...
			if (Main_Order::get_type() == argument::instruction::del::delete_account)
				Log_Recorder::pre_recording_procedure();
			return;
		}
	}
...
{	//사실 여기에 attribute 하나밖에 필요없는데, Account 클래스랑 모양 비슷하게 맞춘것 뿐임.
	...
	argument::order_type op = option::expression::Translation::operation_translate(Main_Order::get_content(argument::operation_position));
