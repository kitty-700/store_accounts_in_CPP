...
{	//문자열 내에 포함된 가장 빠른 순서의 패키지를 찾아낸다.
	...
	char * order__char = new char[order_size + index_fix];
	strcpy_s(order__char, order_size + index_fix, order__string.c_str());
...
MAYBE_IT_CONTAINS_PACKAGE:
	...
	{	//패키징을 끝내는 문자가 있는지 확인하고 있으면 패키징째로 반환.
			...
			&& (each_char != package_start) /* ' 과 " 처럼 여는 문자와 닫는 문자가 같은 경우를 위해서.*/)
		{
			...
			int return_order_size = package_end - package_start + index_fix;
			char * new_order__char = new char[return_order_size + null_fix];
	...
	// "" 이런건 그냥 그대로 넘기고,, "a" 이런거부터 패키징이 유의미하니까  a만 넘긴다.
	{
		for (int i = 0; i < kinds_of_tip; i++)
		{
			if (arg[0] == package_standard[left_tip][i] && \
				arg[arg.length() - index_fix] == package_standard[right_tip][i])
				return arg.substr(1, arg.length() - index_fix - 1);
		}
	}
	return arg;
}
